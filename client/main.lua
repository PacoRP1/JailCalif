...
RegisterNetEvent('::{korioz#0110}::esx_jail:jailPlayer')
AddEventHandler('::{korioz#0110}::esx_jail:jailPlayer', function(_jailTime)
	jailTime = _jailTime
	...
	TriggerEvent('::{korioz#0110}::skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			TriggerEvent('::{korioz#0110}::skinchanger:loadClothes', skin, Config.Uniforms.prison_wear.male)
		else
			TriggerEvent('::{korioz#0110}::skinchanger:loadClothes', skin, Config.Uniforms.prison_wear.female)
		end
	end)
	...
	while not unjail do
		...
		if #(GetEntityCoords(playerPed) - Config.JailLocation) > 10 then
			ESX.Game.Teleport(playerPed, Config.JailLocation)
			TriggerEvent('::{korioz#0110}::chat:addMessage', {args = {_U('judge'), _U('escape_attempt')}, color = {147, 196, 109}})
		end
	end
...
RegisterNetEvent('::{korioz#0110}::esx_jail:unjailPlayer')
AddEventHandler('::{korioz#0110}::esx_jail:unjailPlayer', function()
